...
def get_mappings():
    if not current_app.elasticsearch:
        return
    return current_app.elasticsearch.indices.get_mapping(index=['_all'], doc_type=['_doc'], ignore_unavailable=True)
...
def add_doc_to_index(index, doc):
    ...
    current_app.elasticsearch.index(index=index, doc_type='_doc', id=doc.id, body=body)


def remove_doc_from_index(index, doc):
    if not current_app.elasticsearch:
        return
    current_app.elasticsearch.delete(index=index, doc_type='_doc', id=doc.id)
...
def query_index(index, query, excluded, tags, offset, size, sort):
    ...
    search = current_app.elasticsearch.search(
        index=index, doc_type='_doc', body={
            'query': {
                'bool': {
                    'must': [{
                        'multi_match': {
